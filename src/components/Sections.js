...
export default function Sections({sectionList, setSectionList, handleAddAppClick}){
    ...
    function handleOnDrag(e, appTitle, appImage, sectionIndex){
        e.dataTransfer.setData('text/plain', `${appTitle}\n${appImage}\n${sectionIndex}`);
    ...
    function handleOnDrop(e, sectionIndex){
        ...
        if (draggedSectionIndex !== sectionIndex.toString()) {
            ...
            if (index === parseInt(draggedSectionIndex)) {
                ...
                const newApps = [...section.apps, { title: appTitle, image: appImage }];
                return {
                ...section,
                apps: newApps
                };
    ...
    return(
        <div className="bodyContainer">
            ...
            {sectionList.map((section, index) => ( /**mapping through the sectionList array to access all the section objects which are elements in this array */
                    <div key={index} className="singleSectionContainer" onDrop={(e) => handleOnDrop(e, index)} onDragOver={handleDragOver}>
                        <button className="sectionDropDownBtn" onClick={() => toggleSection(index)}>
                            {isActiveList[index]? 
                        ...
                        {isActiveList[index] && 
                            <AppSection sectionApps={section.apps} handleDeleteAppClick={handleDeleteAppClick} handleOnDrag={handleOnDrag} sectionIndex={index}/>
                        }
                    </div>
            ))}
            </div>
...
function AppSection({sectionApps, handleDeleteAppClick, handleOnDrag, sectionIndex}){
    return(
        <div className="singleSectionApps">
            {sectionApps.map((app, index) => ( /** mapping through the single section apps array to get all apps in one section */
                    <div key={index} className="app"  draggable onDragStart={(e) => handleOnDrag(e, app.title, app.image, sectionIndex)}>
